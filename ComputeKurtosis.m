function [GK,NIG]=ComputeKurtosis(D,V,mask,FileName,sigma)
if(nargin<5)
    sigma=[0.0015 0.0008];
end
...
for ix=1:nx
    parfor iy=1:ny
        [M2_basis,M4_basis]=ConstructM2M4(D(:,:,ix,iy),sigma);
        m2=M2_basis*V(:,ix,iy);
        m4=M4_basis*V(:,ix,iy);
        ...
        [v,d]=eig(M4);d=diag(d);d(d<1e-10)=0;M4=v*diag(d)*v';
        [v,d]=eig(M2);d=diag(d);d(d<2e-5)=2e-5;M2=v*diag(d)*v';
