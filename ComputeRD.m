function [MMSDE,RD]=ComputeRD(D,V,mask,FileName,sigma)
if(nargin<5)
    sigma=[0.0015 0.0008];
end
...
for ix=1:nx
    parfor iy=1:ny
        ...
        R0=D(:,:,ix,iy)/(2*pi^2);
        mmsde(ix,iy)=real(trace(R0+R-2*sqrtm(sqrtm(R0)*R*sqrtm(R0))));
        rd(ix,iy)=real(trace(R0\R+R\R0));
    end
end

MMSDE=zeros(size(mask));
MMSDE(mask~=0)=mmsde*1e6;
...
mat2nhdr(MMSDE, strcat(FileName,'_MMSDE'), 'custom',sd);
