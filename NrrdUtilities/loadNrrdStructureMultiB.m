function headerInfo = loadNrrdStructureMultiB( nhdrFileName )
...
while ( ~feof(fidr) )
  ...
  elseif ( foundKeyword('DWMRI_B-VALUE', cs ) )
    ...
    bvalue = str2num( extractKeyValueString( cs(length('DWMRI_B-VALUE')+1:end ) ) );
  ...
  elseif ( foundKeyword('DWMRI_GRADIENT_', cs ) )
    ...
    headerInfo.bvalue(iGNr+1)=bvalue;
